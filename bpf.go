...
func GetLoadedPrograms() ([]BPFProgInfo, error) {
	...
	for {
		...
		syserr = bpfsys.ObjectGetInfoByFD(&bpfsys.BPFAttrGetInfoFD{
			BPFFD:   fd,
			InfoLen: uint32(bpftypes.BPFProgInfoSize),
			Info:    uintptr(unsafe.Pointer(&info)),
		})
		if syserr != nil {
			return nil, fmt.Errorf("bpf obj_get_info_by_fd returned error: %w", syserr)
		}
		...
		progInfo := BPFProgInfo{
			...
			LoadTime:        bootTime.Add(time.Duration(info.LoadTime)),
