...
func (m *StackMap) Init() error {
	return nil
}

func (m *StackMap) Keys() []RegisterValue {
...
func (m *StackMap) Lookup(key RegisterValue) (RegisterValue, error) {
	...
	// Since the stack grows down(new items at the bottom), index 0 is the last value.
	value := m.Values[len(m.Values)-1-keyIndex]

	return &MemoryPtr{Memory: value, Offset: 0}, nil
}

func (m *StackMap) Push(value RegisterValue, size int64) error {
...
func (m *StackMap) Pop() (RegisterValue, error) {
	if len(m.Values) == 0 {
		return newIMM(0), nil
	}
