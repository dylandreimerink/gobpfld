...
func main() {
	...
	ipStats := program.Maps["ip_proto_stats"].(*gobpfld.BPFGenericMap)
	tcpStats := program.Maps["tcp_stats"].(*gobpfld.BPFGenericMap)
	udpStats := program.Maps["udp_stats"].(*gobpfld.BPFGenericMap)
